...
describe('Chat room service', () => {
	...
	describe('send function', () => {
		it('it emits a `chat message` event type to the socket with relevant message text as value', fakeAsync(inject([ChatRoomService], (chatRoomSvc: ChatRoomService) => {
	...
	describe('messages$ observable', () => {
		...
		it('new messages from socket emit values', async(inject([ChatRoomService], (charRoomSvc: ChatRoomService) => {
			charRoomSvc.connected$.subscribe();
			charRoomSvc.messages$.pipe(skip(1)).subscribe((v) => {
