...
@Component({
	...
	template: `
      <section>
          ...
          <button class="btn" (click)="chatRoom.toggleConnectionStatus()"
                  [ngClass]="{'btn-danger': chatRoom.connected$ | async,'btn-info':!(chatRoom.connected$ | async)}">
              {{(chatRoom.connected$ | async) ? 'Disconnect' : 'Connect'}}
          </button>
...
export class ChatRoomComponent implements OnDestroy,AfterViewInit {
